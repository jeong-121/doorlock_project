...
parameter integer ALARM_TICKS = 10000
)(
input wire clk,
input wire rst,
...
// Alarm timer: ALARM state lasts for a fixed time, then returns to IDLE.
assign alarm_timer_enable = (state == ALARM);

auto_lock_timer #(
    .TIMEOUT_TICKS(ALARM_TICKS)
) ALARM_TIMER (
    .clk(clk),
    .rst(rst),
    ...
    if (rst) begin
        ...
        case (state)
            ...
            ALARM: begin
                ...
                // Alarm is held for 10 seconds only, then system returns to IDLE.
